...
const main = async () => {
  ...
  if (networkName === "testnet" || networkName === "mainnet") {
    ...
    // Check if the LuchowSwap Lottery smart contract address is set.
    ...
    try {
      ...
      const [_blockNumber, _lotteryId, _randomGenerator] = await Promise.all([
      ...
      const tx = await contract.closeLottery(_lotteryId, {from: operator.address});

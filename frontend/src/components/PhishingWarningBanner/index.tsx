...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://app.luchowswap.com - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/app.luchowswap.com)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://app.luchowswap.com'}
          color={text === 'https://app.luchowswap.com' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
